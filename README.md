...
corpus/MANIFEST-*.tsv    URL, version stamp, retrieval date, bytes, SHA-256 per title
tools/fetch-utah-code.py refetch the corpus (all titles, or named ones)
tools/cite.py            pull verbatim section text; search by section
...
```sh
python3 tools/cite.py 23A-4-601                       # verbatim text of a section
python3 tools/cite.py 63G-12                          # a whole chapter
python3 tools/cite.py --grep 'lawful presence'        # sections matching a pattern
python3 tools/cite.py --grep 'perjury' --title 26B    # ...within one title

rg -z 'proof of identity' corpus/utah-code/           # raw search (gzip is transparent to rg -z)
...
- [**Does a Utah fishing license require strong identification?**](findings/fishing-license-probe.md)
  — No. §23A-4-601 conditions issuance on payment of a fee alone. More broadly, Utah's general
...
The corpus is **statute-only**. Two layers are missing, and no "Utah law nowhere requires X" claim is
complete without them:

1. **Utah Administrative Code** — agency rules, where delegated requirements actually live.
   `adminrules.utah.gov` has no bulk download and its rule-listing API resisted access; working
   primitives and the specific blocker are documented in `docs/research-strategy.md` §5.
2. **Federal program conditions** — 42 C.F.R. §435.940 (Medicaid), 7 C.F.R. §273.2 (SNAP),
   6 C.F.R. Part 37 (REAL ID).
